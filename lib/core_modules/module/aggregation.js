...
WeightedList.prototype.addToScalars = function(content){
  ...
  if(content.src){
    if(content.options.hash && content.hash) {
      if (content.options.separator[0] == '?')
        this.src.push((content.options.prefix || '') + content.src + content.options.separator + content.hash);
      else {
        this.src.push((content.options.prefix || '') + content.src.split('.').map(function(item, i, arr) {
          return i == arr.length - 2 ? item + content.options.separator + content.hash : item;
        }).join('.'));
      }
    } else {
      this.src.push((content.options.prefix || '') + content.src);
    }
  }
  if(content.data){
    if(this.data){
...
Asset.prototype.processJSStringData = function(stringdata){
  ...
  if(!this.isGlobal() && this.shouldAggregate()){
...
function configureApp(Meanio,meanioinstance,defer,app){
  Meanio.modules.dependableConstructor().prototype.aggregatePackage = function(){
    ...
    if(Meanio.Singleton.config.clean.assets) {
      ...
      options.prefix = Meanio.Singleton.config.clean.assets.prefix || '';
    } else {
      options.hash = true;
      options.separator = '?v=';
      options.prefix = '';
